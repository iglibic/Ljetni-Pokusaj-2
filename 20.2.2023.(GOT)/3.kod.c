...
int generateUniquePrice(KategorijaP category, int min, int max);                             //dobra ideja CGPT-a, moja jos jednostavnija
...
int printList(KategorijaP first) {
    ...
    while (temp) {
        printf("Kategorija: %s, Prosjecna cijena proizvoda: %f\n", temp->imeKategorije, temp->avgCijena);
...
int removeProductsBlwAvg(KategorijaP head) {                                                        //nisam bas
    KategorijaP temp = head->next;
    while (temp) {
        ProizvodP prev = NULL;
