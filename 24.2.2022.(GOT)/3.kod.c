...
int main() {
	...
	printf("Lista vozila kojima je istekla registracija: \n");
	printList2(head);
...
int loadFromFileVehicles(PersonP head, const char* fileName) {					//ova mi je jebena bila
	FILE* fp = fopen(fileName, "r");
	if (fp == NULL) {
		printf("ERROR! Could not open the file!\n");
		return EXIT_FAILURE;
	...
	while (temp) {
		...
		while (vehicle) {
			if (vehicle->date.year < 2025) {
				...
				found = 1;
				vehicle = vehicle->next;																		//prebaciti svaki put inace beskonacna petlja
			}
			else{
